...
data.2010.prime <- data.2010 %>% distinct(contractid, fips, .keep_all = TRUE)

round_up_counts <- data.2010.prime %>%
...
summary_df <- data.frame(
  ...
  # Part D counts
  PartD_Yes = c(
    mean(ma.rd1 %>% filter(window == TRUE) %>% pull(partd) == "Yes", na.rm = TRUE),
    mean(ma.rd2 %>% filter(window == TRUE) %>% pull(partd) == "Yes", na.rm = TRUE)
  ),
  PartD_No = c(
    mean(ma.rd1 %>% filter(window == TRUE) %>% pull(partd) == "No", na.rm = TRUE),
    mean(ma.rd2 %>% filter(window == TRUE) %>% pull(partd) == "No", na.rm = TRUE)
  ),
  
  # Below/Above threshold
  Below_Threshold = c(
    mean(ma.rd1 %>% filter(window == TRUE) %>% pull(score) < 0, na.rm = TRUE),
    mean(ma.rd2 %>% filter(window == TRUE) %>% pull(score) < 0, na.rm = TRUE)
  ),
  Above_Threshold = c(
    mean(ma.rd1 %>% filter(window == TRUE) %>% pull(score) >= 0, na.rm = TRUE),
    mean(ma.rd2 %>% filter(window == TRUE) %>% pull(score) >= 0, na.rm = TRUE)
...
With a bandwidth of 0.125, we note that there is a substantially higher proportion of below-threshold contracts than above, for both categories. The threshold was calculated via score, i.e, adjusted raw rating, as HMO is constituted through Part C and defined here as score. Furthermore, we see the proportion of Part D plans significantly increasing from 55% to 87%, going from 3-star to 3.5-star comparison. Thus, there are indeed varying characteristics between plans.

## Problem 10

The effect of increased star rating is directly proportional to a larger number of enrollments, particularly prominent from the upward and downward slopes, above and below the threshold, respectively, in the Problem 8 plot comparing 3-star plans against 3.5-star ones. However, these results are highly sensitive to the chosen bandwidth (Problems 6 & 7); regardless, we see that it brings an incremental distribution of the number of plan D contracts going from 3-star threshold to 3.5-star threshold (Problem 9). Further, an effect is added from the fact that the 3-star ratings experience the largest number of round-ups as compared to plans above it, thereby making that region particularly volatile to analysis.
